...
class TCPSender {
  private:
    ...
    //! flying segments to be resent
    std::queue<TCPSegment> _flying_segments{};

    //! total flying bytes
    uint64_t _total_flying_size;

    //! size of remote receive window size 
    //! the window_size function in receiver
    size_t _receive_window_size;

    //! the right edge of remote receive window .
    ...
    uint64_t _consecutive_retransmission;

    //! latest one abs_ackno
    uint64_t _latest_abs_ackno;

    //! abs fin seq
    uint64_t _abs_fin_seq;
